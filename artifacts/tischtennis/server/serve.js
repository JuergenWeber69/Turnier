 ...
 * - GET / without expo-platform → web build (static-build/web/) or landing page
 * Everything else falls through to web build, then static file serving.
 *
...
function serveWebApp(pathname, res) {
  ...
  // If file doesn't exist or is a directory, fall back to index.html (Expo Router static export)
...
const server = http.createServer((req, res) => {
  ...
  // Browser requests — serve web build directly (no Expo Go prompt)
  if (hasWebBuild()) {
    return serveWebApp(pathname, res);
  }

  // Fallback: landing page at root, static files elsewhere
  if (pathname === "/") {
